...
import { ILuisConfig, FileInfo, IBotProject, FeatureFlagMap } from '@botframework-composer/types';

import { AzurePublishErrors } from './utils/errorHandler';
import { BotProjectDeployLoggerType } from './types';

const botPath = (projPath: string, featureFlags: FeatureFlagMap) =>
  featureFlags.NEW_CREATION_FLOW.enabled ? projPath : path.join(projPath, 'ComposerDialogs');
...
export async function publishLuisToPrediction(
  ...
  const luisConfigFiles = (await getFiles(botPath(path, featureFlags))).filter((filename) =>
...
export async function build(
  ...
  builder.rootDir = botPath(path, featureFlags);
  ...
  await builder.copyModelPathToBot(featureFlags);
}
