...
import { isAdditionalField, JSONSchema7, UIOptions } from '@bfc/extension-client';
import fromPairs from 'lodash/fromPairs';
import toPairs from 'lodash/toPairs';

const getSchemaWithAdditionalFields = (baseSchema: JSONSchema7, uiOptions: UIOptions): JSONSchema7 => {
  const additionalFields = toPairs(uiOptions.properties).filter(([, property]) => isAdditionalField(property));
