...
import "./Graph.scss";
import { BoundingBox, Coord2d, EdgeDragData, EdgeDropData, EdgeTargetChangedData, GraphEdge, GraphFlexDirection, LayoutNode, NodeSize, StringMap } from "./GraphSchemas";
...
export interface GraphProps<TData> {
    nodeList: Array<GraphNode<TData>>;
    edgeList: GraphEdge[];
...
export class Graph<TData> extends React.Component<GraphProps<TData>, GraphState> {
    ...
    private prevNodeList: Array<GraphNode<TData>>;
    ...
    private toDictionary(nodeList: Array<GraphNode<TData>>): StringMap<GraphNode<TData>> {
        const dict: StringMap<GraphNode<TData>> = {};
        nodeList.forEach((node) => dict[node.id] = node);
        return dict;
    }

    private populateLayoutGraph(nodeMap: StringMap<GraphNode<TData>>, layoutGraph: Dagre.graphlib.Graph, anchorMap: StringMap<StringMap<Coord2d>>): void {
    ...
    private getNodePropsMap(nodeMap: StringMap<GraphNode<TData>>, edges: GraphEdge[], layoutGraph: Dagre.graphlib.Graph): StringMap<NodeProps<TData>> {
    ...
    private getEdgePropsMap(graphEdges: GraphEdge[], nodePropsMap: StringMap<NodeProps<TData>>, anchorMap: StringMap<StringMap<Coord2d>>): StringMap<EdgeProps<TData>> {
    ...
    private onEdgeAnchorsChanged = (nodeId: string, anchors: StringMap<Coord2d>): void => {
        ...
        const isChanged = anchors && Object.keys(anchors).some((edgeLocalId) => {
    ...
    private getAllNodes(layoutGraph: Dagre.graphlib.Graph): Array<NodeProps<TData>> {
