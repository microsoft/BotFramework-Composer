...
import { Coord2d, EdgeDragAndDropTargetType, EdgeDragData, EdgeDropData, EdgeOptions, GraphFlexDirection } from "./GraphSchemas";
...
export class Edge<TData> extends React.Component<EdgeProps<TData>, EdgeState> {
    ...
    private renderAnchorDropZone(): React.ReactNode {
        if (!this.props.dragData
            || this.props.dragData.dragTargetType !== "Anchor"
            && this.props.dragData.edgeId !== this.props.id) {
            return null;
        }
        ...
        return (
            <div className="circle anchor-drop-zone"
                style={{
                    width: r * 2,
                    height: r * 2,
    ...
    private renderAnchor(): React.ReactNode {
        ...
        return (
            <div className="circle anchor draggable"
                draggable={true}
                style={{
                    width: r * 2,
                    height: r * 2,
    ...
    private renderAttractorPath(receptorPoint: Coord2d, animatedClassName: string, customPathClass: string): React.ReactNode {
        const attractorCenter = this.getAttractorCenter();

        if (!receptorPoint
            || (receptorPoint.x === attractorCenter.x && receptorPoint.y === attractorCenter.y)) {
            return null;
        }
    ...
    private renderCurve(d: string, virtualClassName: string, animatedClassName: string, customPathClass: string): React.ReactNode {
        return (
            <path className={`path ${virtualClassName} ${animatedClassName} ${customPathClass}`} d={d} onAnimationEnd={this.onAnimationEnd} />
        );
    }

    private renderAnchorAttachmentStraightline(virtualClassName: string, animatedClassName: string, customPathClass: string): React.ReactNode {
    ...
    private renderCubicBezierCurve(curveEnd: Coord2d, virtualClassName: string, animatedClassName: string, customPathClass: string): React.ReactNode {
    ...
    private getStartControlPointForHorizontalFlex(curveStart: Coord2d, curveEnd: Coord2d, midPoint: Coord2d): Coord2d {
        ...
        const x = isTargetLeftOfSource
            ? curveStart.x + Constants.edgeControlPointOffset
            : midPoint.x;
    ...
    private getStartControlPointForVerticalFlex(curveStart: Coord2d, curveEnd: Coord2d, midPoint: Coord2d): Coord2d {
        ...
        const y = isTargetAboveSource
            ? curveStart.y + Constants.edgeControlPointOffset
            : midPoint.y;
    ...
    private getEndControlPointForHorizontalFlex(curveStart: Coord2d, curveEnd: Coord2d, basePoint: Coord2d): Coord2d {
        ...
        if ((curveStart.y > curveEnd.y && y > curveStart.y)
            || (curveStart.y < curveEnd.y && y < curveStart.y)) {
    ...
    private getEndControlPointForVerticalFlex(curveStart: Coord2d, curveEnd: Coord2d, basePoint: Coord2d): Coord2d {
        ...
        if ((curveStart.x > curveEnd.x && x > curveStart.x)
            || (curveStart.x < curveEnd.x && x < curveStart.x)) {
