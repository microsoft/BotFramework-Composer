...
const yeomanWork = async (
  ...
  parentPort?.postMessage({ status: 'Getting environment' });
  ...
  parentPort?.postMessage({ status: 'Installing template' });
  ...
  parentPort?.postMessage({ status: 'Instantiating template' });
