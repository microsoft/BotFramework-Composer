...
export interface PublishPlugin<Config = any> {
    ...
    rollback?: (config: Config, project: IBotProject, rollbackToVersion: string, user?: UserIdentity) => Promise<PublishResponse>;
    schema?: JSONSchema7;
    instructions?: string;
    customName?: string;
...
export declare const DEFAULT_RUNTIME = "csharp-azurewebapp";
export interface RuntimeTemplate {
    ...
    buildDeploy: (runtimePath: string, project: IBotProject, settings: DialogSetting, profileName: string) => Promise<string>;
    /** set skill manifest, different folder for different runtime  */
    setSkillManifest: (dstRuntimePath: string, dstStorage: IFileStorage, srcManifestDir: string, srcStorage: IFileStorage, mode: string) => Promise<void>;
