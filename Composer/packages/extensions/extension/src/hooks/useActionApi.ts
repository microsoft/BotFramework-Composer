...
export const useActionApi = (shellApi: ShellApi) => {
  ...
  function constructActions(dialogId: string, actions: BaseSchema[]) {
    ...
    const referenceLuIntent: FieldProcessorAsync<any> = (fromId, fromAction, toId, toAction) => {
      fromAction[luFieldName] && createLuIntent(dialogId, fromAction[luFieldName], toId, toAction);
  ...
  function copyActions(dialogId: string, actions: BaseSchema[]) {
    // 'SendActivity_1234' -> '- hi'
    const dereferenceLg: FieldProcessorAsync<string> = (fromId, fromAction, toId, toAction, lgFieldName) =>
      readLgTemplate(dialogId, fromAction[lgFieldName]);

    // 'TextInput_Response_1234' -> LuIntentSection | undefined
    const dereferenceLu: FieldProcessorAsync<any> = (fromId, fromAction, toId, toAction) => {
  ...
  function constructAction(dialogId: string, action: BaseSchema) {
    return constructActions(dialogId, [action]);
  }

  function copyAction(dialogId: string, action: BaseSchema) {
    return copyActions(dialogId, [action]);
  }

  function deleteAction(dialogId: string, action: BaseSchema) {
  ...
  function deleteActions(dialogId: string, actions: BaseSchema[]) {
