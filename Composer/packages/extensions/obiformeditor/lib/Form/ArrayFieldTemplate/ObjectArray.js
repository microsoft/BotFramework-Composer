...
var ObjectArray = function(props) {
  ...
  var objectProperties = useMemo(
    function() {
      return Object.keys(properties).filter(isVisible);
    },
    [properties]
  );
  return jsx(
    ...
    object &&
      jsx(
        ...
        objectProperties.length > 1 &&
          objectProperties.map(function(key, index) {
      ...
      canAdd &&
        (!object
          ? jsx(
              ...
              jsx(
                DefaultButton,
                { type: 'button', onClick: onAddClick, 'data-testid': 'ArrayContainerAdd' },
                formatMessage('Add')
              )
            )
          : jsx(
              'div',
              { css: arrayItemInputFieldContainer },
              objectProperties.map(function(property, index, items) {
                return jsx(
                  'div',
