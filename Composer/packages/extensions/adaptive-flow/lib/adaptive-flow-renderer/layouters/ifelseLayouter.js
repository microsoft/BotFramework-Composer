...
export function ifElseLayouter(conditionNode, choiceNode, ifNode, elseNode) {
    if (!conditionNode || !choiceNode)
        return new GraphLayout();
    var containerBoundary = calculateIfElseBoundary(conditionNode.boundary, choiceNode.boundary, ifNode.boundary, elseNode.boundary);
    var leftNode = ifNode || new GraphNode();
    var rightNode = elseNode || new GraphNode();
    var leftNodeText = formatMessage('True');
    var rightNodeText = formatMessage('False');
    // Condition
    ...
    var flag = (leftNode ? '1' : '0') + (rightNode ? '1' : '0');
    switch (flag) {
        case '11':
            /**
             *     <Condition>
            ...
            rightNode.offset = {
                x: Math.max(choiceNode.offset.x + choiceNode.boundary.width, leftNode.offset.x + leftNode.boundary.width) +
                    BranchIntervalX,
                y: choiceNode.offset.y + choiceNode.boundary.height + BranchIntervalY,
            };
            break;
...
//# sourceMappingURL=ifelseLayouter.js.map