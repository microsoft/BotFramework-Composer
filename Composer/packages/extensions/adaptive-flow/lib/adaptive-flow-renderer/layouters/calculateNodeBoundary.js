...
export function calculateSequenceBoundary(boundaries, widthHeadEdge, widthTailEdge) {
  if (widthHeadEdge === void 0) {
    widthHeadEdge = true;
  }
  if (widthTailEdge === void 0) {
    widthTailEdge = true;
  }
  ...
  box.axisX = Math.max.apply(
    Math,
    __spreadArrays(
      [0],
      boundaries.map(function (x) {
        return x.axisX;
      })
    )
  );
...
function measureBranchingContainerBoundary(conditionBoundary, choiceBoundary, branchBoundaries) {
  ...
  branchGroupBoundary.height = Math.max.apply(
    Math,
    branchBoundaries.map(function (x) {
      return x.height;
    })
  );
