...
import { StandardNodeWidth, HeaderHeight, DiamondSize, InitNodeSize, LoopIconSize, ChoiceInputSize, ChoiceInputMarginTop, ChoiceInputMarginBottom, IconBrickSize, AssignmentMarginTop, PropertyAssignmentSize, AssignmentMarginBottom, } from '../constants/ElementSizes';
...
import { calculateIfElseBoundary, calculateSequenceBoundary, calculateSwitchCaseBoundary, calculateForeachBoundary, calculateBaseInputBoundary, } from './calculateNodeBoundary';
function measureStepGroupBoundary(stepGroup) {
    var boundaries = (stepGroup.children || []).map(function (x) { return measureJsonBoundary(x); });
...
function measureForeachBoundary(json) {
    ...
    var inputs = [foreachDetail, stepGroup, loopBegin, loopEnd].map(function (x) { return measureJsonBoundary(x.json); });
...
function measureIfConditionBoundary(json) {
    ...
    if (!result)
        return new Boundary();
    var condition = result.condition, choice = result.choice, ifGroup = result.ifGroup, elseGroup = result.elseGroup;
    var inputs = [condition, choice, ifGroup, elseGroup].map(function (x) { return measureJsonBoundary(x.json); });
...
function measureSwitchConditionBoundary(json) {
    ...
    return calculateSwitchCaseBoundary(measureJsonBoundary(condition.json), measureJsonBoundary(choice.json), branches.map(function (x) { return measureJsonBoundary(x.json); }));
}
export function measureChoiceInputDetailBoundary(data) {
    var width = InitNodeSize.width;
    var height = InitNodeSize.height +
...
export function measurePropertyAssignmentBoundary(data) {
    var width = InitNodeSize.width;
    var height = Math.max(InitNodeSize.height / 2 +
        (data.assignments && Array.isArray(data.assignments)
            ...
            : 0), InitNodeSize.height);
    return new Boundary(width, height);
}
function measureBaseInputBoundary(data) {
    var _a = transformBaseInput(data, ''), botAsks = _a.botAsks, userAnswers = _a.userAnswers;
