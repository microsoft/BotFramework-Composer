...
export var calculateBranchNodesIntervalX = function (leftNodeBound, rightNodeBound) {
    if (!rightNodeBound)
        return 0;
    return Math.max(BranchIntervalX, BranchAxisXIntervalMin - getRightWidth(leftNodeBound) - getLeftWidth(rightNodeBound));
};
export var getLeftWidth = function (bound) { return (bound ? bound.axisX : 0); };
export var getRightWidth = function (bound) { return (bound ? bound.width - bound.axisX : 0); };
