...
var calculateArrowPoints = function (endPoint, direction, arrowSize) {
    if (arrowSize === void 0) { arrowSize = 5; }
    switch (direction) {
        case EdgeDirection.Up:
            return [
...
export var drawSVGEdge = function (id, x, y, direction, length, options) {
    ...
    var directed = edgeOptions.directed, color = edgeOptions.color, dashed = edgeOptions.dashed, label = edgeOptions.label, labelOptions = edgeOptions.labelOptions;
    var strokeProps = {
    ...
    // Draw the edge line
    if (length > 0) {
        var line = (React.createElement("line", __assign({ key: "edge-" + id + "__line", x1: startPoint.x, x2: endPoint.x, y1: startPoint.y, y2: endPoint.y }, strokeProps)));
        elements.push(line);
    }
    // Draw the edge label
    if (typeof label === 'string' || typeof label === 'number') {
        var text = (React.createElement("text", { key: "edge-" + id + "__text", fontSize: labelOptions === null || labelOptions === void 0 ? void 0 : labelOptions.fontSize, x: startPoint.x + (((_a = labelOptions === null || labelOptions === void 0 ? void 0 : labelOptions.offset) === null || _a === void 0 ? void 0 : _a.x) || 0), y: startPoint.y + (((_b = labelOptions === null || labelOptions === void 0 ? void 0 : labelOptions.offset) === null || _b === void 0 ? void 0 : _b.y) || 0) }, label));
        elements.push(text);
    }
    // Draw the edge arrow
    if (directed) {
        var _c = calculateArrowPoints(endPoint, direction), p1 = _c[0], p2 = _c[1];
        var points = [p1, endPoint, p2].map(function (p) { return p.x + "," + p.y; }).join(' ');
        var arrow = (React.createElement("polyline", __assign({ key: "edge-" + id + "__arrow", points: points }, strokeProps, { fill: "none", strokeDasharray: "none" })));
        elements.push(arrow);
    }
    return elements;
};
export var renderEdge = function (edge) {
    var id = edge.id, x = edge.x, y = edge.y, direction = edge.direction, length = edge.length, options = edge.options;
