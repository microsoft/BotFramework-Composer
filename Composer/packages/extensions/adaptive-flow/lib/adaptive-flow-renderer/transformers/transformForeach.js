...
export function transformForeach(input, jsonpath) {
    if (!input || (input.$kind !== AdaptiveKinds.Foreach && input.$kind !== AdaptiveKinds.ForeachPage))
        return null;
    var foreachDetailNode = new IndexedNode(jsonpath, __assign(__assign({}, input), { $kind: input.$kind === AdaptiveKinds.ForeachPage ? AdaptiveKinds.ForeachPageDetail : AdaptiveKinds.ForeachDetail }));
