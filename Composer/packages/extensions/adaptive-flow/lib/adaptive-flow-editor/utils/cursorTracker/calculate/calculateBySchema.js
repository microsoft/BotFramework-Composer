...
function parseSelector(path) {
    ...
    var normalizedSelectors = selectors.reduce(function (result, selector) {
        ...
        if (parseResult) {
            var objSelector = parseResult[1], arraySelector = parseResult[2];
    ...
    switch (direction) {
        case Direction.Up:
            if (currentElement.tab === PromptTab.OTHER || currentElement.tab === PromptTab.USER_INPUT) {
                candidateElements = elements.filter(function (ele) { return ele.selectedId === "" + currentElement.focusedId + PromptTab.BOT_ASKS; });
            }
            break;
        case Direction.Down:
            if (currentElement.tab === PromptTab.USER_INPUT) {
                candidateElements = elements.filter(function (ele) { return ele.tab !== PromptTab.OTHER; });
            }
            break;
        case Direction.Left:
            if (currentElement.tab === PromptTab.OTHER) {
                candidateElements = elements.filter(function (ele) { return ele.tab === PromptTab.USER_INPUT; });
            }
            break;
        case Direction.Right:
            if (!currentElement.tab) {
                ...
                candidateElements = elements.filter(function (ele) { return ele.selectedId === "" + currentElement.focusedId + PromptTab.OTHER; });
            }
            break;
        default:
...
function handleNextMoveFilter(currentElement, elements) {
    ...
    return elements.filter(function (ele) {
        ...
        var condition1 = eleSelectors.length === currentElementSelectors.length &&
        ...
        var condition2 = eleSelectors.length > currentElementSelectors.length &&
            eleSelectors.join('.').includes(currentElementSelectors.join('.')) &&
            Number(eleSelectors[eleSelectors.length - 1]) === 0;
        var condition3 = eleSelectors.length < currentElementSelectors.length &&
...
function handleSwitchCasePrevMoveFilter(currentElement, elements) {
    ...
    var sortedElement = elements
        .filter(function (ele) {
        var eleSelectors = parseSelector(transformDefaultBranch(ele.selectedId));
        return (eleSelectors.slice(0, swicthPosition).join('.') === samePath &&
