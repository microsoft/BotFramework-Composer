...
export var useSelectionEffect = function (state, shellApi) {
    var data = state.data, nodeContext = state.nodeContext;
    var focusedEvent = nodeContext.focusedEvent, focusedId = nodeContext.focusedId;
    var onSelect = shellApi.onSelect, onFocusSteps = shellApi.onFocusSteps;
    var _a = useState([]), selectedIds = _a[0], setSelectedIds = _a[1];
    var _b = useState(querySelectableElements()), selectableElements = _b[0], setSelectableElements = _b[1];
    var nodeIndexGenerator = useRef(new NodeIndexGenerator());
    var getSelectableIds = function () { return nodeIndexGenerator.current.getItemList().map(function (x) { return x.key; }); };
    useEffect(function () {
    ...
    var selection = new Selection({
        onSelectionChanged: function () {
            ...
            var selectedIds = selectedIndices.map(function (index) { return nodeItems[index].key; });
    ...
    var getNodeIndex = function (nodeId) { return nodeIndexGenerator.current.getNodeIndex(nodeId); };
    return {
        selection: selection,
