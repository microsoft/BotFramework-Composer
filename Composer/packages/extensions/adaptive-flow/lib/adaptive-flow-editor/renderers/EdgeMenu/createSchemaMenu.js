...
var createBaseActionMenu = function (menuSchema, onClick, filter) {
  ...
  var menuTree = Object.entries(menuSchema).reduce(function (result, _a) {
    var $kind = _a[0],
      options = _a[1];
    if (filter && !filter($kind)) return result;
  ...
  var buildMenuItemFromMenuTree = function (labelName, labelData) {
    if (typeof labelData === 'string') {
      ...
      var subMenuItems = Object.keys(labelData)
        ...
        .map(function (sublabelName) {
          return buildMenuItemFromMenuTree(sublabelName, labelData[sublabelName]);
        });
      return createSubMenu(labelName, onClick, subMenuItems);
    }
  };
  var stepMenuItems =
    ((_a = buildMenuItemFromMenuTree('root', menuTree).subMenuProps) === null || _a === void 0 ? void 0 : _a.items) ||
    [];
...
export var createActionMenu = function (onClick, options, menuSchema, customActionGroups) {
  ...
  if (customActionGroups) {
    ...
    var is$kindUngrouped = function ($kind) {
      return !menuOptions[$kind];
    };
    ...
    if (customActionItems.length) {
      ...
      var submenuWithDuplicatedName = resultItems.find(function (item) {
        return item.text === customActionGroupName_1;
      });
      if (submenuWithDuplicatedName) {
        // When 'Custom Actions' label exists, append custom actions after that submenu.
        (_b = submenuWithDuplicatedName.subMenuProps) === null || _b === void 0
          ? void 0
          : (_a = _b.items).push.apply(_a, customActionItems);
      } else {
