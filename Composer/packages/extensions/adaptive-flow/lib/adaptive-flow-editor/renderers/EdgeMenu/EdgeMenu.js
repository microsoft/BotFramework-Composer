...
export var EdgeMenu = function (_a) {
    ...
    var _b = useContext(NodeRendererContext), clipboardActions = _b.clipboardActions, customSchemas = _b.customSchemas;
    ...
    var _c = useState(false), menuSelected = _c[0], setMenuSelected = _c[1];
    ...
    var menuItems = createActionMenu(function (item) {
        if (!item)
            return;
        onClick(item.key);
    }, {
    ...
    customSchemas.map(function (x) { return x.oneOf; }).filter(function (oneOf) { return Array.isArray(oneOf) && oneOf.length; }));
    ...
    return (jsx("div", __assign({ style: {
        ...
        jsx(TooltipHost, { content: moreLabel, directionalHint: DirectionalHint.rightCenter },
            jsx(IconMenu, { handleMenuShow: handleMenuShow, iconName: "Add", iconSize: 7, iconStyles: {
                    color: '#005CE6',
                    selectors: {
                        ':focus': {
                            outline: 'none',
                ...
                }, label: moreLabel, menuItems: menuItems, nodeSelected: nodeSelected }))));
};
