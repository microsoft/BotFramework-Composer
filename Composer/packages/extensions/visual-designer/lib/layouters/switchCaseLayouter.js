...
export function switchCaseLayouter(conditionNode, choiceNode, branchNodes) {
  if (branchNodes === void 0) {
    branchNodes = [];
  }
  if (!conditionNode) {
    return new GraphLayout();
  }
  var containerBoundary = calculateSwitchCaseBoundary(
    conditionNode.boundary,
    choiceNode.boundary,
    branchNodes.map(function (x) {
      return x.boundary;
    })
  );
  ...
  branchNodes.reduce(function (accOffsetX, x, currentIndex) {
    ...
    return (
      accOffsetX +
      x.boundary.width +
      calculateBranchNodesIntervalX(
        x.boundary,
        (_a = branchNodes[currentIndex + 1]) === null || _a === void 0 ? void 0 : _a.boundary
      )
    );
