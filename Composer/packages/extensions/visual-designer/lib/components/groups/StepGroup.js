...
var getStepKey = function (stepOrder) {
  return 'steps[' + stepOrder + ']';
};
var parseStepIndex = function (stepKey) {
  return parseInt(stepKey.replace(/steps\[(\d+)\]/, '$1'));
};
var calculateNodes = function (groupId, data) {
  var steps = transformStepGroup(data, groupId);
  var stepNodes = steps.map(function (x) {
    return GraphNode.fromIndexedJson(x);
  });
...
var calculateLayout = function (nodeMap) {
  var nodes = Object.keys(nodeMap)
    .sort(function (a, b) {
      return parseStepIndex(a) - parseStepIndex(b);
    })
    .map(function (stepName) {
      return nodeMap[stepName];
    });
  return sequentialLayouter(nodes);
};
export var StepGroup = function (_a) {
  var id = _a.id,
    data = _a.data,
    onEvent = _a.onEvent,
    onResize = _a.onResize;
  var initialNodes = useMemo(
    function () {
      return calculateNodes(id, data);
    },
    [id, data]
  );
  var _b = useSmartLayout(initialNodes, calculateLayout, onResize),
    layout = _b.layout,
  ...
  return jsx(
    ...
    jsx(
      SVGContainer,
      { hidden: true, height: boundary.height, width: boundary.width },
    ...
    nodes
      ? nodes.map(function (node, index) {
          return jsx(
            OffsetContainer,
            { key: 'stepGroup/' + node.id + '/offset', offset: node.offset },
            jsx(StepRenderer, {
      ...
      ? nodes.map(function (x, idx) {
          return jsx(
            ...
            jsx(EdgeMenu, {
              ...
              id: id + '[' + (idx + 1) + ']',
              onClick: function ($kind) {
                return onEvent(NodeEventTypes.Insert, { id: id, $kind: $kind, position: idx + 1 });
              },
            })
          );
        })
