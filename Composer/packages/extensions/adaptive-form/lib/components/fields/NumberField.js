...
var NumberField = function (props) {
    var description = props.description, disabled = props.disabled, id = props.id, label = props.label, onChange = props.onChange, readonly = props.readonly, schema = props.schema, value = props.value, required = props.required, uiOptions = props.uiOptions;
    var type = schema.type;
    var updateValue = function (step) { return function (value) {
        if (value === '') {
            onChange(0);
            return;
        }
    ...
    return (react_1.default.createElement(react_1.default.Fragment, null,
        ...
        react_1.default.createElement(SpinButton_1.SpinButton, { ariaLabel: label || format_message_1.default('numeric field'), decrementButtonAriaLabel: format_message_1.default('decrement by { step }', { step: step }), disabled: Boolean(schema.const) || readonly || disabled, id: id, incrementButtonAriaLabel: format_message_1.default('increment by { step }', { step: step }), step: step, styles: {
                labelWrapper: { display: 'none' },
            }, value: displayValue, onDecrement: updateValue(-step), onIncrement: updateValue(step), onValidate: updateValue(0) })));
};
