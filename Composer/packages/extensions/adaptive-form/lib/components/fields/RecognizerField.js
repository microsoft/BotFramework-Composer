...
var RecognizerField = function (props) {
  ...
  var _a = extension_1.useShellApi(),
    shellApi = _a.shellApi,
    shellData = tslib_1.__rest(_a, ['shellApi']);
  var recognizers = extension_1.useRecognizerConfig();
  var qnaFiles = shellData.qnaFiles,
    luFiles = shellData.luFiles,
    currentDialog = shellData.currentDialog,
    locale = shellData.locale;
  var _b = react_1.useState(false),
    isCustomType = _b[0],
    setIsCustomType = _b[1];
  react_1.useEffect(
    function () {
      // this logic is for handling old bot with `recognizer = undefined'
      if (value === undefined) {
        var qnaFile = qnaFiles.find(function (f) {
          return f.id === currentDialog.id + '.' + locale;
        });
        var luFile = luFiles.find(function (f) {
          return f.id === currentDialog.id + '.' + locale;
        });
        if (qnaFile && luFile) {
  ...
  var options = react_1.useMemo(
    function () {
      ...
      return recognizers
        .filter(function (r) {
          return r.id !== shared_1.SDKKinds.LuisRecognizer;
        })
        .map(function (r) {
          return {
            key: r.id,
  ...
  var selectedType = react_1.useMemo(
    function () {
      if (isCustomType) {
      ...
      var involvedCustomItem = selected.find(function (item) {
        return item !== shared_1.SDKKinds.CustomRecognizer;
      });
  ...
  var handleChangeRecognizerType = function (_, option) {
    ...
    if (option) {
      ...
      var handler =
        (_a = recognizers.find(function (r) {
          return r.id === option.key;
        })) === null || _a === void 0
          ? void 0
          : _a.handleRecognizerChange;
      if (handler) {
  ...
  return core_1.jsx(
    ...
    selectedType
      ? core_1.jsx(Dropdown_1.Dropdown, {
          ...
          label: format_message_1.default('Recognizer Type'),
          options: options,
          responsiveMode: Dropdown_1.ResponsiveMode.large,
          selectedKey: selectedType,
          onChange: handleChangeRecognizerType,
        })
      : format_message_1.default('Unable to determine recognizer type from data: {value}', { value: value }),
    selectedType === shared_1.SDKKinds.CustomRecognizer &&
