...
function getOptionLabel(schema, parent) {
    var title = schema.title, enumOptions = schema.enum, items = schema.items;
    if (title) {
...
function getOptions(schema, definitions) {
    var type = schema.type, oneOf = schema.oneOf, enumOptions = schema.enum;
...
function getSelectedOption(value, options) {
    ...
    else if (expressionOption &&
        valueType === 'string' &&
        (value.startsWith('=') || !options.find(function (_a) {
            var key = _a.key;
            return key === 'string';
        }))) {
        return expressionOption;
