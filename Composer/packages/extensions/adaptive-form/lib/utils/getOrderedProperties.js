...
function getOrderedProperties(schema, baseUiOptions, 
// eslint-disable-next-line @typescript-eslint/no-explicit-any
data) {
    var _a;
    var uiOptions = cloneDeep_1.default(baseUiOptions);
    var _b = uiOptions.order, order = _b === void 0 ? ['*'] : _b;
    ...
    var allProperties = Object.keys((_a = schema.properties) !== null && _a !== void 0 ? _a : {}).filter(function (p) { return !p.startsWith('$') && !hiddenFieldSet.has(p); });
    var restIdx = orderedFields.indexOf('*');
    // only validate wildcard if not all properties are ordered already
    if (allProperties.some(function (p) { return !orderedFieldSet.has(p); })) {
        var errorMsg = void 0;
        if (restIdx === -1) {
        ...
        if (errorMsg) {
            throw new Error(format_message_1.default('Error in UI schema for {title}: {errorMsg}\n{options}', {
                title: schema.title,
                errorMsg: errorMsg,
