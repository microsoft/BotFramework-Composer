...
function getUiPlaceholder(props) {
  ...
  if (fieldUIPlaceholder && schema.pattern) {
    ...
    var placeholderExamples = fieldUIPlaceholder.split(',').map(function (example) {
      return example.trim();
    });
    var filteredExamples = placeholderExamples.filter(function (example) {
      return example.match(regex_1);
    });
