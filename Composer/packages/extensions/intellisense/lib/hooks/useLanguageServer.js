...
exports.useLanguageServer = function (url, scopes, documentUri, textFieldValue, cursorPosition, projectId) {
  ...
  var _a = react_1.default.useState([]),
    completionItems = _a[0],
    setCompletionItems = _a[1];
  ...
  react_1.default.useEffect(
    function () {
      ...
      ws.current.onopen = function () {
        ...
        ws.current.send(
          JSON.stringify(
            lspMessagesUtils_1.getTextDocumentOpenedMessage(
              documentUri,
              LANGUAGE_NAME,
  ...
  var updateBackendMemory = function (newValue) {
    ...
    if (ws.current.readyState === WebSocket.OPEN) {
      ws.current.send(
        JSON.stringify(
          lspMessagesUtils_1.getDocumentChangedMessage(newValue, documentUri, latestDocumentVersion.current)
        )
      );
    }
  };
  ...
  var getCompletionItems = function () {
    ...
    ws.current.send(
      JSON.stringify(
        lspMessagesUtils_1.getCompletionRequestMessage(latestMessageId.current, documentUri, {
          line: 0,
