...
exports.IntellisenseTextField = react_1.default.memo(function (props) {
    var url = props.url, scopes = props.scopes, projectId = props.projectId, id = props.id, value = props.value, onChange = props.onChange, children = props.children;
    var _a = react_1.default.useState(''), textFieldValue = _a[0], setTextFieldValue = _a[1];
    var _b = react_1.default.useState(false), showCompletionList = _b[0], setShowCompletionList = _b[1];
    var _c = react_1.default.useState(0), selectedCompletionItem = _c[0], setSelectedCompletionItem = _c[1];
    var _d = react_1.default.useState(0), cursorPosition = _d[0], setCursorPosition = _d[1];
    ...
    var completionItems = useLanguageServer_1.useLanguageServer(url, scopes, id, textFieldValue, cursorPosition, projectId);
    ...
    react_1.default.useEffect(function () {
        var outsideClickHandler = function (event) {
            ...
            if (mainContainerRef.current && completionListRef.current) {
                if (uiUtils_1.checkIsOutside(x, y, mainContainerRef.current) && uiUtils_1.checkIsOutside(x, y, completionListRef.current)) {
                    setShowCompletionList(false);
                }
            }
        };
    ...
    var setValueToSelectedCompletionItem = function (index) {
        if (index < completionItems.length) {
            ...
            if (range) {
                var newValue = textFieldValue.substr(0, range.start.character) +
                    selectedSuggestion +
    ...
    var onKeyUpMainComponent = function (event) {
        switch (event.key) {
            case 'ArrowDown':
                (completionItems === null || completionItems === void 0 ? void 0 : completionItems.length) && setSelectedCompletionItem(function (index) { return (index + 1) % (completionItems === null || completionItems === void 0 ? void 0 : completionItems.length); });
                break;
            case 'ArrowUp':
                (completionItems === null || completionItems === void 0 ? void 0 : completionItems.length) &&
                    setSelectedCompletionItem(function (index) { return ((completionItems === null || completionItems === void 0 ? void 0 : completionItems.length) + index - 1) % (completionItems === null || completionItems === void 0 ? void 0 : completionItems.length); });
                break;
            case 'Enter':
    ...
    return (react_1.default.createElement("div", { onKeyUp: onKeyUpMainComponent, ref: mainContainerRef, style: { position: 'relative' } },
        children(textFieldValue, onValueChanged, onKeyDownTextField, onKeyUpTextField, onClickTextField),
        showCompletionList && (react_1.default.createElement(CompletionList_1.CompletionList, { ref: completionListRef, completionItems: completionItems, selectedItem: selectedCompletionItem, onClickCompletionItem: onClickCompletionItem }))));
});
