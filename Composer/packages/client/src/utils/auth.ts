...
const MAX_WAIT = 1000 * 60 * 10; // 2 minutes
...
export async function loginPopup(url = '', callbackUrl = ''): Promise<string | undefined> {
  ...
  var armToken = await authClient.getToken([arm]);
  var graphToken = await authClient.getToken([graph]);

  // Test code: Graph - get my user profile
  var headers = new Headers();
  var bearer = 'Bearer ' + graphToken;
  ...
  var headers = new Headers();
  var bearer = 'Bearer ' + armToken;
  headers.append('Authorization', bearer);
  var options = {
    method: 'GET',
    headers: headers,
  };
  var graphEndpoint = 'https://management.azure.com/subscriptions?api-version=2020-01-01';
