...
const options = [{ key: 'azure', text: 'AzureBlobStorage' }];
...
export default function AddStoragePanel(props) {
  ...
  const updateInfo = field => (e, newValue) => {
    ...
    setStorageData({ ...storageData, [field]: value });
  };

  const handleSubmit = e => {
    e.preventDefault();

    onSubmit({ ...storageData, id: new Date().getTime() });
  };

  return (
    <div style={{ paddingLeft: '10px', width: '600px' }}>
      <form onSubmit={handleSubmit}>
        ...
        <Button onClick={handleSubmit} type="submit" primary styles={{ root: { width: '100%', marginTop: '20px' } }}>
          Submit
        </Button>
      </form>
    </div>
