...
export const PublishController = {
  ...
  publish: async (req, res) => {
    ...
    const { subscriptionKey } = sensitiveSettings.qna || '';
    const qnaEndpointKey =
      subscriptionKey && !currentProject.settings?.qna.endpointKey
        ? await currentProject.updateQnaEndpointKey(subscriptionKey)
        : '';
