...
import { Coord2d, EdgeDragData, EdgeDropData, GraphEdge, GraphFlexDirection, LayoutNode, NodeSize, StringMap } from "./GraphSchemas";
...
type ContainerContentComponentType<TData> = React.ComponentClass<NodeContentProps<TData>> & { getWrappedInstance: () => VanillaContentComponentType<TData> };
...
export function Node<TData>(ContentComponent: ContentComponentType<TData>): React.ComponentClass<NodeProps<TData>> {
    return class extends React.Component<NodeProps<TData>> implements NodeRenderer {
        ...
        private renderAttractorDropZone(): React.ReactNode {
            ...
            return (
                <div className="attractor-drop-zone"
                    style={{
                        borderRadius: r,
                        width: r * 2,
                        height: r * 2,
        ...
        private getEdgeAnchorPoint = (edge: GraphEdge): Coord2d | undefined => {
            ...
            const contentComponent: NodeContent<any> =
                (this.contentRef.current as any).getWrappedInstance
                    ? (this.contentRef.current as any).getWrappedInstance()
                    : this.contentRef.current;

            let anchorPoint = contentComponent.getEdgeAnchorPoint(edge) as AnchorPointResult;

            if (anchorPoint) {
                if (anchorPoint === "NodeDefault") {
        ...
        private reportNodeSize(): void {
            ...
            const threshold = 10;
            const isSignificant = Math.abs(nodeSize.width - this.props.width) > threshold || Math.abs(nodeSize.height - this.props.height) > threshold;
        ...
        private getAttractorSpaitalStyle(): React.CSSProperties {
            ...
            const left = isVertical
                ? this.props.width / 2 - Constants.edgeAttractorRadius
                : - Constants.edgeAttractorRadius;
            const top = isVertical
                ? -Constants.edgeAttractorRadius
                : this.props.height / 2 - Constants.edgeAttractorRadius;
